...
export const getCashFlowStatement = async (query: string) => {
    try {
      const data = await axios.get<CompanyCashFlow[]>(
        `https://financialmodelingprep.com/api/v3/cash-flow-statement/${query}?limit=40&apikey=YAJ0TxfOj8le8a8ChEMjbLJ3BqpFBIfN`
      );
      return data;
    } catch (error: any) {
      console.log("error message: ", error.message);
  ...
  export const getTenK = async (query: string) => {
    try {
      const data = await axios.get<CompanyTenK[]>(
        `https://financialmodelingprep.com/api/v3/sec_filings/${query}?type=10-k&page=0&apikey=YAJ0TxfOj8le8a8ChEMjbLJ3BqpFBIfN`
      );
      return data;
    } catch (error: any) {
      console.log("error message: ", error.message);
